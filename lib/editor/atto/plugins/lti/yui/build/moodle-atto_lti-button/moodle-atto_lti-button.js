...
Y.namespace('M.atto_lti').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _addTool: function(event, tool) {
        ...
        document.CALLBACKS['f' + resourceLinkId] = function(response) {
            var items = response.items || [];
            console.log('Called back.', items);

            panel.hide();
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                var strategyFactory = new Y.M.atto_lti.PlacementStrategyFactory();
                var strategy = strategyFactory.strategyFor(item, courseid, tool);
...
Y.namespace('M.atto_lti').PlacementStrategyFactory = function() {
    ...
    this.strategyFor = function(item, course, tool) {
        ...
        var strategy = new StrategyClass(item, course, tool);
...
Y.namespace('M.atto_lti').LTIRenderingStrategy = function(item) {
    console.log('rendering lti for', item);
    var mode = "embed";
    ...
    var template = Y.Handlebars.compile('<a href="{{item.ltiurl}}" data-lti="{{mode}};width:{{width}}px;height:{{height}}px" target="{{target}}">{{item.text}}</a>');
