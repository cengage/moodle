...
class mod_lti_coursenav_lib_testcase extends advanced_testcase {
    ...
    /**
     * Tests the course placements filtered by course for course tools.
     */
    public function test_course_placements_course_tool() {
        $this->resetAfterTest();
        $course = $this->getDataGenerator()->create_course();
        $config = new stdClass();
        $config->lti_organizationid = '';
        $type = create_type($config, $course);
        $menulinks = [
            "link1"=>["label"=> "menulink1"],
            "link2"=>["label"=> "menulink2", "url"=>"https://somewhere", "allowlearners"=>"1"]
        ];
        ...
        $this->assertEquals(1, count($coursenavsaddible));
        $this->assertEquals(2, count($coursenavsaddible[$type->id]->menulinks));
        // It's only for this course, so other should not see those nav links.
        $coursenavsaddible = lti_coursenav_lib::get()->load_coursenav_links($course->id+1);
        $this->assertEquals(0, count($coursenavsaddible));
    }
